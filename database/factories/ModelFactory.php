...
$factory->define(StockFarmacia::class, function (Faker\Generator $faker) {
    
    $Farmacia = Farmacias::all()->random();
    
    
    return [
     'Cantidad'        => $faker->numberBetween($min = 50, $max = 100),
     'Tipo_accion'     => $faker->numberBetween($min = 1, $max = 5),
     'Fecha'           => $faker->date($format = 'Y-m-d', $max = 'now'),
     'idFarmacia'      => $Farmacia->id,
     'idCentro_medico' => $Farmacia->idCentro_medico,
     'Estado'          => $faker->randomElement([StockFarmacia::ACTIVO, StockFarmacia::NO_ACTIVO]),
     ];
});


$factory->define(Estudios::class, function (Faker\Generator $faker) {
    
    $Paciente = Pacientes::all()->random(); 
    return [
     'Fecha'           => $faker->date($format = 'Y-m-d', $max = 'now'),
     'Tipo_estudio'    => $faker->numberBetween($min = 1, $max = 5),
     'Descripcion'     => $faker->paragraph,
     'idPaciente'      => $Paciente->id,
     'idCentro_medico' => $Paciente->idCentro_medico,
     'Estado'          => $faker->randomElement([Estudios::ACTIVO, Estudios::NO_ACTIVO]),
     ];
});